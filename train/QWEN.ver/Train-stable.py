...
CONFIG = {
    ...
    "CHECKPOINT_DIR": "../../HOMOboa-model/checkpoints",
    "MAX_LENGTH": 512,
    "LOAD_IN_8BIT": True,

    "DATA_PATH": "../../json",
    "DATA_FILE": "diary_boa_KAfuck.jsonl",
    "BATCH_SIZE": 1,
...
# ==================== 数据加载 ====================
dataset = load_dataset(CONFIG["DATA_PATH"], data_files={"train": CONFIG["DATA_FILE"]})["train"]
...
train_loader = DataLoader(dataset, batch_size=CONFIG["BATCH_SIZE"], sampler=sampler, collate_fn=data_collator)

# ==================== 模型加载 ====================
model = AutoModelForCausalLM.from_pretrained(CONFIG["BASE_MODEL"], load_in_8bit=CONFIG["LOAD_IN_8BIT"], device_map="auto")
peft_cfg = LoraConfig(
    r=CONFIG["LORA_R"], lora_alpha=CONFIG["LORA_ALPHA"], target_modules=CONFIG["LORA_TARGET_MODULES"],
    lora_dropout=CONFIG["LORA_DROPOUT"], bias=CONFIG["LORA_BIAS"], task_type="CAUSAL_LM"
)
...
# ==================== 绘图函数（按 epoch 平均） ====================
def plot_metrics(metrics_per_epoch, save_path):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    epochs = list(range(1, len(metrics_per_epoch["loss"])+1))
    fig, axes = plt.subplots(2,3, figsize=(18,10))
    axes = axes.flatten()

    # Loss
    axes[0].plot(epochs, metrics_per_epoch["loss"], marker="o")
    axes[0].set_title("Average Loss per Epoch")
    axes[0].set_xlabel("Epoch"); axes[0].set_ylabel("Loss")

    # Perplexity
    axes[1].plot(epochs, metrics_per_epoch["perplexity"], marker="o", color="orange")
    axes[1].set_title("Average Perplexity per Epoch")
    axes[1].set_xlabel("Epoch"); axes[1].set_ylabel("Perplexity")

    # Gradient
    axes[2].plot(epochs, metrics_per_epoch["grad_norms"], marker="o", color="green")
    axes[2].set_title("Average Gradient Magnitude per Epoch")
    axes[2].set_xlabel("Epoch"); axes[2].set_ylabel("Grad Value")

    # Learning Rate
    axes[3].plot(epochs, metrics_per_epoch["lr"], marker="o", color="red")
    axes[3].set_title("Average LR per Epoch")
    axes[3].set_xlabel("Epoch"); axes[3].set_ylabel("LR")

    # Step Time
    axes[4].plot(epochs, metrics_per_epoch["step_time"], marker="o", color="purple")
    axes[4].set_title("Average Step Time per Epoch")
    axes[4].set_xlabel("Epoch"); axes[4].set_ylabel("Seconds")

    # Throughput
    axes[5].plot(epochs, metrics_per_epoch["throughput"], marker="o", color="brown")
    axes[5].set_title("Average Throughput per Epoch")
    axes[5].set_xlabel("Epoch"); axes[5].set_ylabel("Samples/sec")
...
for epoch in range(CONFIG["EPOCHS"]):
        ...
        if (step + 1) % CONFIG["GRADIENT_ACCUMULATION_STEPS"] == 0:
            ...
            for p in model.parameters():
                if p.grad is not None:
                    grad_vals.append(p.grad.detach().abs().mean().item())
            grad_mean = sum(grad_vals)/len(grad_vals) if grad_vals else 0.0
            epoch_grad_norm += grad_mean
    ...
    metrics["grad_norms"].append(epoch_grad_norm / len(train_loader))
    ...
    print(f"Epoch {epoch+1} | avg_loss: {metrics['loss'][-1]:.4f} | time: {epoch_elapsed:.1f}s | speed: {epoch_elapsed/len(train_loader):.2f}s/step")

    # 保存模型
    epoch_dir = os.path.join(CONFIG["CHECKPOINT_DIR"], f"epoch_{epoch+1}")
    os.makedirs(epoch_dir, exist_ok=True)
    unwrapped = accelerator.unwrap_model(model)
    unwrapped.save_pretrained(epoch_dir, safe_serialization=True)
    tokenizer.save_pretrained(epoch_dir)
    print(f"💾 模型已保存至 {epoch_dir}")

    # 绘图
    metrics_path = os.path.join(CONFIG["CHECKPOINT_DIR"], "metrics_images", f"metrics_up_to_epoch_{epoch+1}.png")
